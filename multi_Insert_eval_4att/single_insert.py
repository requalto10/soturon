...
from IL_evaluation import IL_calc
...
# 本番データ
df = pd.read_csv(f'./insurance_data/result/inserted_k={K}.csv', names=cols)
add_rec = pd.read_csv('./insurance_data/evaluation/to_insert.csv', names=cols)
...
df_new_raw = pd.concat([original, add_rec], ignore_index=True)

print('Finish!')
df.to_csv(f'./insurance_data/result/inserted_k={K}.csv', header=False, index=None)
df_new_raw.to_csv(f'./insurance_data/evaluation/raw_4att.csv', header=False, index=None)
IL_calc(K)