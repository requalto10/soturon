...
del_rec = pd.read_csv(f'./insurance_data/evaluation/to_delete.csv', names=cols)
...
if ungroup_blk_size > K:
    ...
    for i in range(ungroup_blk_size - 1):
        ...
        add_rec_index = anonymized_add_rec.iat[0, -2]
...
df.to_csv(f'./insurance_data/result/deleted_k={K}.csv', header=False, index=None)
